...
namespace ConsoleApp8
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < array科目名稱.Length; i += 1) //長度=5，所以要<長度
            {
            ...
            {
                Console.WriteLine(subject); //看不到索引值，但不會產生例外錯誤
            }
            ...
            for (int i = 0; i <= array2D_Two.GetUpperBound(0); i += 1) //GetUpperBound(0)第一維度的索引值上限(從0開始，所以小長度1) 
            {
                for (int j = 0; j <= array2D_Two.GetUpperBound(1); j += 1)
                {
                    Console.WriteLine("array2D_Two[{0}, {1}] = {2};", i, j, array2D_Two[i,j]);
                }
            }
            Console.WriteLine("-------------------------------------");
            ...
            for (int i = 0; i <= array3D_One.GetUpperBound(0); i += 1) //GetUpperBound(0)第一維度的索引值上限(從0開始，所以小長度1) 
            {
            ...
            bool is有這科目 = false; //系統化命名，前面加型態關鍵字
